...
const VERSION = process.env.VORTEX_VERSION || "1.0.0";
const APP_UNPACKED = resolve(REPO_ROOT, "dist", "linux-unpacked");
const RPM_OUTPUT = resolve(REPO_ROOT, "dist");
const ICON_SOURCE = resolve(REPO_ROOT, "assets", "images", "vortex.png");
...
const rpmTopDir = mkdtempSync(resolve(tmpdir(), "vortex-rpm-"));

try {
  for (const sub of ["BUILD", "RPMS", "SPECS", "SOURCES"]) {
    mkdirSync(resolve(rpmTopDir, sub), { recursive: true });
  }

  const specPath = resolve(rpmTopDir, "SPECS", "vortex.spec");
  writeFileSync(specPath, SPEC);

  console.log(`Building RPM (version ${VERSION})...`);
  ...
  console.log(`RPM written to ${RPM_OUTPUT}/`);
} finally {
